...
export const openMysteryBox = async (userId, boxType) => {
  ...
  // }));
  let chosenCategoryItem = { type: "collectible", chance: 100 }; // Default to collectible since it's the only category now
    ...
    case "collectible":
      ...
      if (!chosenTierItem) {
        ...
        //console.error(`Error selecting collectible tier for ${boxType} box.`);
         throw new Error(`Tier selection failed for ${boxType}`);
        //no fallback now
        // Fallback: Give coins
